...
const TechnologyForm = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (response.data.metadata) {
                ...
                if (data.icon_url) {
                    setExistingImage(`http://localhost:5000/uploads/technologies/${data.icon_url}`);
                }
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditMode) {
                await api.put(`/technologies/${id}`, submitData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                await api.post('/technologies', submitData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }
    ...
    return (
        <div className="admin-module">
            <div className="module-header">
                <h2>{isEditMode ? 'Sửa Công nghệ' : 'Thêm mới Công nghệ'}</h2>
                <button className="btn-secondary" onClick={() => navigate('/admin/technologies')}>Quay lại</button>
            </div>
                ...
                <div className="form-section">
                    <div className="form-row">
                        <div className="form-group">
                            <label>Tên công nghệ (VD: React, Node.js) *</label>
                            <input type="text" name="name" value={formData.name} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label>Phân loại (VD: Frontend, Database)</label>
