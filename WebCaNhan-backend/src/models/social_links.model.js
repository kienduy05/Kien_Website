...
class SocialLinksModel {
    static async getAll() {
        ...
        const result = await pool.request().query('SELECT * FROM social_links');
        return result.recordset;
    }
    
    static async getById(id) {
        const pool = await getSqlPool();
        const result = await pool.request().input('id', sql.INT, id).query('SELECT * FROM social_links WHERE id = @id');
    ...
    static async create(data) {
        ...
        const query = `INSERT INTO social_links (${columns}) OUTPUT INSERTED.* VALUES (${values})`;
    ...
    static async update(id, data) {
        ...
        const query = `UPDATE social_links SET ${setString} OUTPUT INSERTED.* WHERE id = @id`;
        const result = await request.query(query);
        return result.recordset[0];
    }
    ...
    static async delete(id) {
        const pool = await getSqlPool();
        const result = await pool.request().input('id', sql.INT, id).query('DELETE FROM social_links WHERE id = @id');
