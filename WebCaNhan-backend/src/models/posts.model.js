...
class PostsModel {
    ...
    static async create(data) {
        ...
        const query = `INSERT INTO posts (${columns}) OUTPUT INSERTED.* VALUES (${values})`;
    ...
    static async update(id, data) {
        ...
        const query = `UPDATE posts SET ${setString} OUTPUT INSERTED.* WHERE id = @id`;
        const result = await request.query(query);
        return result.recordset[0];
    }
    ...
    static async delete(id) {
        const pool = await getSqlPool();
        const result = await pool.request().input('id', sql.INT, id).query('DELETE FROM posts WHERE id = @id');
