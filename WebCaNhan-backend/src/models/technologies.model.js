...
class TechnologiesModel {
    ...
    static async getById(id) {
        const pool = await getSqlPool();
        const result = await pool.request().input('id', sql.INT, id).query('SELECT * FROM technologies WHERE id = @id');
    ...
    static async create(data) {
        ...
        const query = `INSERT INTO technologies (${columns}) OUTPUT INSERTED.* VALUES (${values})`;
    ...
    static async update(id, data) {
        ...
        const query = `UPDATE technologies SET ${setString} OUTPUT INSERTED.* WHERE id = @id`;
        const result = await request.query(query);
        return result.recordset[0];
    }
    ...
    static async delete(id) {
        const pool = await getSqlPool();
        const result = await pool.request().input('id', sql.INT, id).query('DELETE FROM technologies WHERE id = @id');
